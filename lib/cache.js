...
const { GhostNoMoreServersError, GhostUnknownNodeError, GhostKeyError } = require('./ghost_error');
...
class Cache {
    ...
    */
            ...
            if (nodes[i].node == server) {
                ...
                return request.get(address)
                    .catch((err) => {
                        ...
                        throw new GhostUnknownNodeError();
                    });
           }
        }
        throw new GhostUnknownNodeError();
    ...
    /*
    The `nodeSize()` method will return the number of key-value
    pairs in the specified server.
    */
    async nodeSize(server) {
        var nodes = this.ring.getPoints();
        for (var i = 0; i < nodes.length; i++) {
            if (nodes[i].node == server) {
                var address = this.protocol + nodes[i].node + ':' + this.port + _APIEndpoints["nodeSize"];
                return request.get(address)
                    .catch((err) => {
                        _DeadServers.add(nodes[i].node);
                        this.ring.delete(nodes[i].node);
